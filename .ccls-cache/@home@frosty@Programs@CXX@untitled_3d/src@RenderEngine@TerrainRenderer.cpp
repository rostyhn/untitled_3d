...
void TerrainRenderer::RenderTerrain(Terrain& terrain)
{
	...
	//https://learnopengl.com/Advanced-OpenGL/Instancing (Render all 9 at the same time!)

        glm::mat4 transform = Maths::CreateTransformMatrix(glm::vec3(terrain.GetX(), 0, terrain.GetZ()), glm::vec3(0, 0, 0), glm::vec3(1, 1, 1));
	// Load the transformation matrix into the shader
	m_shader.LoadTransformMatrix(transform);
