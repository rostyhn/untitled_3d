...
public:
  ...
  void LoadToVAO(std::vector<glm::vec3> vertices, std::vector<glm::vec2> textures,
		 ...
		 const std::string& name, AABB& bBox);
  ...
  AABB& GetBoundingBox(const std::string& modelName);
protected:
  Loader();
 private:
  std::map<const std::string, AABB> m_boundingBoxes;
  ...
  void StoreBoundingBox(const std::string& name, AABB& bBox);
